...
// Last update Thu Jan 15 09:28:26 2015 SOMASUNDRAM Barathan
//
  ...
  if (this->Energy > 10)
    {
      ...
      if (*this->battleData >= xp)
	{
	  stat = new std::string(*task);
	  *stat = *stat + " - Completed!";
	  this->setStatus(stat);
	  *this->battleData = *this->battleData + xp / 2;
	  return (true);
	}
      stat = new std::string(*task);
      *stat = *stat + " - Failed!";      
      this->setStatus(stat);
      *this->battleData = *this->battleData + xp;
      return (false);
    }
  stat = new std::string("Battery Low");
  this->Energy = 0;
  this->setStatus(stat);
  return (false);
}
