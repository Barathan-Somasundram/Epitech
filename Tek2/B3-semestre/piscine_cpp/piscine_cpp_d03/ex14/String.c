...
** Last update Sat Jan 10 09:58:57 2015 SOMASUNDRAM Barathan
*/
...
static String*	split_s(String* this, char separator)
{
  ...
  if ((tab = malloc(sizeof(*tab) * (count + 1))) == NULL)
    return (NULL);
  i = 0;
  while (count > 0)
    {
      (tab[i]).str = strdup(strsep(&(this->str), &separator));
      i++;
      count--;
    }
  return (tab);
...
static char**	split_c(String* this, char separator)
{
  ...
  while (count > 0)
    {
      tab[i] = strdup(strsep(&(this->str), &separator));
      i++;
      count--;     
    }
  tab[i] = NULL;
...
static void	insert_c(String *this, size_t pos, char const* str)
{
  ...
  if (pos > strlen(this->str))
    return;
  if ((tmp = malloc(sizeof(char) * (strlen(this->str) + strlen(str) + 1))) == NULL)
    return;
  if ((tmp = strncat(tmp, this->str, pos)) == NULL)
    return;
  if ((tmp = strcat(tmp, str)) == NULL)
    return;  
  if ((tmp = strcat(tmp, &(this->str[pos]))) == NULL)
    return;
}
...
static void	insert_s(String *this, size_t pos, String const* str)
{
  ...
  if (pos > strlen(this->str))
    return;
  if ((tmp = malloc(sizeof(char) * (strlen(this->str) + strlen(str->str) + 1))) == NULL)
    return;
  if ((tmp = strncat(tmp, this->str, pos)) == NULL)
    return;
  if ((tmp = strcat(tmp, str->str)) == NULL)
    return;  
  if ((tmp = strcat(tmp, &(this->str[pos]))) == NULL)
    return;
}
...
static int	find_s(String *this, const String *str, size_t pos)
{
  ...
  if (strlen(str->str) > strlen(this->str))
    return (-1);
  if ((tmp = strstr(&(this->str[pos]), str->str)) == NULL)
    return (-1);
  while (tmp != &(this->str[pos]))
    pos++;
  return (pos);
}
